...
warn "^^ conn id ^^";
warn "^^ conn id ^^";
use DDP; warn p $self->{state}{conns}{$conn_id}; # conexao e dados do usuário

# send commands to on behalf of $conn_id user
    #change user nick
##  $self->send_event(
##      'cmd_nick',
##      $conn_id,
...
#       {
#           command => 'PRIVMSG',
#           params  => ['#some_chan_forced','!!!!!!!!!!!!!!!!!!!!!! AAAAAAAAAAA pspspspss'],
#       },
#   );
    
    $self->send_event(
        'daemon_cmd_privmsg', #aparentemente não importa o que esteja escrito aqui. deve serpara chamar o evento..
        $conn_id,
        {
            command => 'PRIVMSG',
            params  => ['#some_chan_forced','============> MENSAGEM AUTOMATICA #1'],
        },
    );

    #talk on behalf of user
    $self->send_event(
        'cmd_privmsg',
        $conn_id,
        {
            command => 'PRIVMSG',
            params  => ['#some_chan_forced','============> MENSAGEM AUTOMATICA #2'],
        },
    );
    ...
    $self->send_event(
        ...
        {
            command => 'PRIVMSG',
            params  => ['#some_chan_forced','============> MENSAGEM AUTOMATICA #3'],
        },
    );
...
sub find_nick {
    my $self = shift;
    my $nick = shift;
    return $self->{state}{peers}{uc $self->server_name()}{users}->{uc $nick};
}
...
sub _daemon_cmd_message {
    ...
    warn "_daemon_cmd_message" x 100;
...
XX
                $self->send_event(
                    "daemon_".($type eq 'PRIVMSG' ? 'public' : 'notice'),
                    $full,
                    $channel,
                    $args->[1],
                );
...
sub add_spoofed_nick {
    ...
    if (!defined $record->{ircname}) {
        $record->{ircname} = "* spoofed * I'm too lame to read the documentation *";
    }
    $self->{state}{stats}{invisible}++ if $record->{umode} =~ /i/;
    $self->{state}{stats}{ops_online}++ if $record->{umode} =~ /o/;
